function BillMedia_Update_Gas() {
    ...
    if (parsedNumberMatch && parsedNumberMatch.length > 1) {
        ...
        let accountCardXpath = "//*[contains(text(), '" + liveGasAccountNumber + "')]/ancestor::div[contains(@class, 'card') or contains(@class, 'account') or contains(@class, 'row')][1]";
        let accountCardObj = page.FindElement(accountCardXpath);
        
        if (!accountCardObj.Exists) {
            accountCardObj = page.FindElement("//*[contains(text(), '" + liveGasAccountNumber + "')]");
        }
        ...
        if (accountCardObj.Exists) {
            ...
            Log.Checkpoint("PASSED: Successfully clicked on the Gas account card layout");
        } else {
            Log.Error("Validation Failed: Could not locate a clickable element containing account number " + liveGasAccountNumber);
        }
        ...
        let emailRadioInput = activePage.FindElement("//input[@id='deliveryTypeEP_" + liveGasAccountNumber + "']");
        let postRadioInput = activePage.FindElement("//input[@id='deliveryTypeP_" + liveGasAccountNumber + "']");

        let emailRadioLabel = activePage.FindElement("//label[@for='deliveryTypeEP_" + liveGasAccountNumber + "']");
        let postRadioLabel = activePage.FindElement("//label[@for='deliveryTypeP_" + liveGasAccountNumber + "']");
        ...
        if (emailRadioInput.checked) {
            ...
            if (emailRadioLabel.Exists) {
                ...
                let emailField = activePage.FindElement("//input[@name='de_" + liveGasAccountNumber + "']");
                let confirmEmailField = activePage.FindElement("//input[@name='cde_" + liveGasAccountNumber + "']");
        ...
        let updateDeliveryBtnXpath = "//button[contains(@class, 'btnupdate-billing-delivery" + liveGasAccountNumber + "')]";
  ...
  function Verify_HUBCX_Billing_Delivery(myEngieSelectedMethod) {
    ...
    let targetAccountNumber = Project.Variables.SelectedGasAccountNumber; 
    
    let  exactInputxpath= "//*[@id='searchCustomer:frmGenericMDForm:genericMDDynaFormComp:_ACCTNO']";
    let exactInputField=hubCxPage.WaitElement(exactInputxpath,10000);
...
function Verify_Salesforce_Billing_Delivery(myEngieSelectedMethod) {
  ...
  sfPage.Refresh();
    ...
    sfPage.Wait(); 
  var accountNumber =Project.Variables.SelectedGasAccountNumber;
  ...
  if (closedSearchBox.Exists)
  {
    ...
    let sfFieldObj = sfPage.FindElement(sfFieldQuery);
     if (sfFieldObj.Exists) {
        ...
        Log.Error("Validation Failed: Could not locate the 'Bill Media' text element cell within this Salesforce console layout view.");
    }
