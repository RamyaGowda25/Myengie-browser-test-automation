function Salesforce_Saved_Gas_OOP()
{
  ...
  if (closedSearchBox.Exists)
  {
    ...
    if (activeInputBox.Exists)
    {
      ...
      if (accountLink.Exists)
      {
        ...
        var paymentMethodXPath = "//*[text()='Receipt Payment Method']/ancestor::*[self::div or self::slot or contains(@class,'item')]//*[contains(text(), 'Paypal') or contains(text(), 'AMEX') or contains(text(), 'Mastercard') or contains(text(), 'VISA')] | //*[text()='Receipt Payment Method']/following::*[contains(@class, 'value') or @role='gridcell']//*[contains(text(), 'Paypal') or contains(text(), 'AMEX') or contains(text(), 'Mastercard') or contains(text(), 'VISA')]";
        
        // Dynamic fallback line if Salesforce structure flattens during quick page updates
        if (!page.FindElement(paymentMethodXPath).Exists) {
            paymentMethodXPath = "//*[text()='Paypal' or text()='AMEX' or text()='Mastercard' or text()='VISA'] | //*[contains(text(), 'Paypal') or contains(text(), 'AMEX') or contains(text(), 'Mastercard') or contains(text(), 'VISA')]";
        }
        ...
        if (paymentMethodField.Exists) {
            ...
            // Clean checkpoint verification logging whichever active card/method was found live on screen
            Log.Checkpoint("PASSED: Receipt Payment Method validation successful! Detected valid active type: '" + actualPaymentMethod + "'.");
          aqUtils.Delay(3000);
        } else {
            Log.Error("Fail: Unable to resolve or locate a valid payment method matching 'Paypal', 'AMEX', 'Mastercard', or 'Visa' under the field header container.");
        }   
 ...
 //-- Click on Related tab
       ...
       //--Click on Cases
        ...
        if (resolutionField.Exists) {
            ...
            var expectedResolutionText = "Successful Payment Request (Saved Method)";
