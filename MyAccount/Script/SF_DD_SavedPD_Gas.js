function Salesforce_VISA_ELEC_DD()
{
 
  let browser = Sys.Browser("chrome");
  let page = browser.Page("*simplyenergy--preprod.sandbox*");
  var accountNumber =Project.Variables.DirectDebitGasAccountNumber;
      ...
      if (accountLink.Exists)
      {
       ...
       // 9. REVISED PRECISION CHECKPOINT FOR PAYPAL VALUE
        ...
        if (paymentMethodElement.Exists)
        {
          ...
          var expectedPaymentMethod = "Direct Credit";
        ...
        // --- CHECKPOINT 2: Credit Card Type Verification ---
        var ccTypeValueXPath = "//*[text()='Credit Card Type']/ancestor::*[contains(@class, 'slds-form-element')]//lightning-formatted-text[@data-output-element-id='output-field']";
        var ccTypeElement = page.FindElement(ccTypeValueXPath);
        if (ccTypeElement.Exists)
        {
          var actualCreditCardType = ccTypeElement.contentText.trim();
          Log.Message("Found Credit Card Type field. Actual text: '" + actualCreditCardType + "'");
          if (actualCreditCardType === "VISA" || actualCreditCardType === "AMEX" || actualCreditCardType === "MasterCard")
          {
            Log.Checkpoint("PASSED Checkpoint: Credit Card Type verified successfully as 'MasterCard'");
          }
          else
          {
            Log.Error("FAILED Checkpoint: Expected Credit Card Type to be 'MasterCard' but found '" + actualCreditCardType + "'");
          }
        }
